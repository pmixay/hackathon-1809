...
Оракул (полное знание будущей траектории) считается тем же целевым правилом и даёт нижнюю границу
затрат; разность «политика − оракул» и есть цена незнания будущего.
...
def run_policy(case, a, spec: dict, fixed: Plan, oracle: bool) -> tuple[dict, list[dict]]:
    ...
    `oracle=True`  — те же формулы, но с фактической будущей долей: нижняя граница затрат.
    """
    shares = spec["shares"]
...
def main() -> None:
    ...
    for key, spec in TRAJECTORIES.items():
        ...
        o_plan = build_policy_plan(case, fixed, o_committed, f"{PLAN}_oracle_{key}",
                                   f"EXP-14: оракул (известная будущая траектория) {spec['label']}")
        o_res = simulate(case, o_plan, sc, a)
        rows.append(kpi_row(o_res, experiment="EXP-14", trajectory=spec["label"], policy="оракул (знает будущее)"))
    ...
    lines = ["# EXP-14. Замкнутая политика реакции: решения только по доступной информации", "",
             ...
             "## Результат", "",
             "| Траектория | Доли ISRU 2038/2039/2040 | Без реакции | Политика | Оракул | Цена незнания, млн |",
             "|---|---|---|---|---|---:|"]
    ...
    lines += ["",
              ...
              f"Цена незнания будущего (политика − оракул): от {min(cost):+,.1f} до {max(cost):+,.1f} млн у.е., "
              f"в среднем {sum(cost) / len(cost):+,.1f} млн.", "",
