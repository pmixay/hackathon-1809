...
def simulate(case: Case, plan: Plan, scenario: Scenario, assumptions: Optional[Assumptions] = None) -> Result:
    ...
    for o in plan.orders:
        ...
        lt, earliest_order, _ = order_terms(s)
        ...
        if first_month is not None and first_month - lt < earliest_order:
            add("LEAD_TIME_VIOLATED", "hard", f"{s.name}: поставка в {ym_str(first_month)} требует заказа не позднее {ym_str(first_month - lt)} "
                f"(срок поставки {lt} мес.), то есть раньше самой ранней допустимой даты заказа {ym_str(earliest_order)}",
    ...
    for idx in sorted(schedule):
        for k, q in sorted(schedule[idx].items()):
            if q <= EPS:
                continue
            s_ = case.sources[k]
            lt, earliest, note_ = order_terms(s_)
            placement = idx - lt
        ...
        if c.metric in ("total_service_level", "critical_service_level"):
            ...
            for yr in year_records:
                if yr.emergency_share_of_demand > emergency_threshold:
                    ...
                    cur = []
            if best > c.value:
                add(c.constraint_id, "hard", f"Emergency является базовым каналом (> {emergency_threshold:.0%} спроса) {best} года подряд {best_years}, лимит {int(c.value)}",
    ...
    for c in case.constraints.values():
        ...
        if c.metric in ("total_service_level", "critical_service_level"):
            ...
            for yr in year_records:
                mrow(c.constraint_id, yr.year, "emergency_share_of_demand", yr.emergency_share_of_demand, emergency_threshold, "<= (иначе год считается базовым для Emergency)",
                     not any(v.rule_id == c.constraint_id and v.year == yr.year for v in viol), "hard", f"не более {int(c.value)} базовых лет подряд")
    if scenario.loss_ceiling_enabled:
