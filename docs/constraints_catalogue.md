...
| RESERVE_45D | hard | начало каждого года | физический режим: `I_start(y) ≥ D_y × 45/365`; контрактный режим: дополнительно `I_start ≥ D_y × 42/365` (6 недель ожидания) и зарезервированный Emergency ≥ R_y | кейс §Начальный запас и резерв; правило эквивалентности = TEAM_ASSUMPTION | год, запас, R_y, недостача, детали эквивалентности |
| STORAGE_OVERFLOW | hard | каждый месяц | `I_end(месяц) ≤ ёмкость(активный режим хранилища)`; начальный запас также проверяется на 2035-01 | кейс §Потери и хранение (ёмкость на выбранном шаге) | год, месяц, запас, ёмкость, превышение |
...
| CAPACITY_EXCEEDED | hard | источник × год | `резерв_т/год ≤ мощность` | контрольный пример V08 | источник, год, резерв, мощность, превышение |
...
| EMERGENCY_BASE_STREAK | hard | горизонт | год считается «Emergency — базовый канал», когда заказ у Emergency > 20 % спроса (допущение); `базовых лет подряд ≤ 2` | кейс §Требования; порог = TEAM_ASSUMPTION | годы, длина серии, лимит |
