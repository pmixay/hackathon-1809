...
- **Для R2 (проверка присланных материалов R1, 19.09)**: числа стратегии P2z совпадают с `results/` (BASE PV 8 729 / 8 639 / 8 858; адаптированный стресс 10 097 / 10 637 / 10 578; дефицит без адаптации 96 / 170 / 170 т; CAPEX 540 / 1 430 / 1 790; P4 дороже P2z на 128 в BASE и 481 в стрессе; MCDA balanced пересчитан: 77,5 / 29,3 / 16,0 против 77,6 / 29,3 / 16,1 — расхождение от округления входов). Требуют правки: (1) триггер «спрос ≥ BASE × 1,05» — план P2z без изменений нарушает 45-дневный резерв уже при × 1,01 (нулевой запас прочности, как и у P3), поэтому триггер должен быть «любой пересмотр прогноза вверх» или заложен буфер; (2) триггер «Earth-New > 24 мес.» противоречит EXP-09: задержка уже на 3 месяца нарушает резерв 2038–2040, реакцию нужно бронировать заранее; (3) адаптированный P2z в BASE даёт 17 нарушений переполнения (EXP-02) — в записке нужно сказать, что выбор плана зависит от наблюдаемого сценария; (4) веса профилей critical-service и financier не раскрыты (нет `configs/mcda_profiles.yaml`); (5) `apply_r2_updates.py` не применим к текущему `main`: 8 из 8 якорей в `management_note.md` не найдены (тексты изменены R3 и перенесены на другие строки), заголовок §3 архитектуры теперь русский, регулярное выражение для `sources.md` §D не совпадает, файлы-спутники (`*_final.md`, `mcda_profiles.yaml`) не приложены; полная замена `stakeholders.md` и `roadmap_budget.md` затрёт правки R3 от 19.09; (6) `R2_FINAL_STRATEGY.md` на английском — по правилу «Язык» документы для жюри на русском (брошюра PDF уже на русском).
