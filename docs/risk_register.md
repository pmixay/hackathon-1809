...
Реестр ведётся в машиночитаемом виде: событие, причина, затронутые параметры, период, основание оценки,
зависимости, владелец, меры с их стоимостью и остаточный риск хранятся как структурированные записи, а
количественное последствие каждого риска задано не числом, а рецептом пересчёта. При сборке материалов
расчётный контур выполняет все десять рецептов на данных кейса и сверяет результат с опубликованным
значением: расхождение выше объявленного допуска останавливает сборку, поэтому число в этом документе
не может устареть незаметно. Сводная таблица с результатом пересчёта прилагается к материалам расчётов.
...
|---|---|---|---|---|---|---|---|
| Р1 | Задержка ввода Earth-New на 3 / 6 / 12 мес. | ввод 2037-01 → 2037-04 / 2037-07 / 2038-01; последствия 2038–2040 | сценарные задержки (TEAM_ASSUMPTION), без вероятности | недопоставлено 3,297 / 6,594 / 13,188 т; сервис 100 %; **45-дневный резерв нарушен в 2038–2040**, разрыв до 3,148 / 6,297 / 12,812 т (EXP-09) | реакция Earth-Flex после наблюдения: 3,214 / 6,428 / 13,078 т, **+30,590 / +61,181 / +124,474 млн PV**; буфер заранее: +34,678 / +69,346 / +141,068 млн PV, оплачивается и без задержки (EXP-13); защитный календарь Earth-New (апрель–декабрь 2037, резерв 17,585 т/год): +0,285 млн PV без задержки, +6,712 при +3 мес., при +6/+12 мес. не защищает (EXP-12) | задержка, объявленная после 2037-08, не оставляет 4 месяцев до проверки резерва; задержка свыше 12 мес. не рассчитана | ПОСТ C / ОП |
| Р2 | Спрос выше базового (стресс +15 % с 2038; высокий вариант +10…+25 %) | 2038–2040 (стресс); 2035–2040 (высокий вариант) | обязательный стресс и официальный высокий вариант | план без изменений в стрессе: **дефицит 95,917 т**, общий сервис 0,870 (2040), резерв нарушен 2038–2040; в высоком варианте: дефицит 234,917 т, резерв нарушен во все годы; уже +1 % спроса нарушает резерв 2038–2040, +10 % даёт дефицит 47,917 т (EXP-02, EXP-03, EXP-12) | заблаговременная адаптация заказов и запаса: дефицит 0, нарушений 0, **+954,574 млн PV** к неизменному плану в стрессе; перепланирование под высокий спрос: PV 10 238,585, дефицит 2,557 т, сервис ≥ 99,48 % | у фиксированного графика нулевой запас прочности — резерв требует ежегодного пересчёта; адаптированный график в BASE переполняет хранилище (17 нарушений) | ОП / ПОТР |
...
| Р5 | Задержка ввода ZBO (лаг 0–12 мес.) | 2037-07 → до 2038-07 | диапазон допущения (TEAM_ASSUMPTION) | годовое отношение потерь к поступлению превышает 2 % при лаге 10 мес.; лаги 7–9 мес. проходят (EXP-04); для P2z разрывы 45-дневного резерва 1,676 / 3,352 / 7,672 т при задержке 3 / 6 / 12 мес. в BASE, дефицита нет (EXP-17); до модернизации ёмкость 70 т ограничивает буферы | реакция Earth-Flex 1,733 / 3,511 / 8,033 т за +10,832 / +22,268 / +54,100 млн PV (заказ к 2037-01); более раннее решение о ZBO — 0 / 0 / 12,346 млн PV при задержке и премия 5,885 / 12,029 / 35,928 млн без неё; приёмочные испытания и неустойка в договоре (предложено) | при задержке 12 мес. в обязательном стрессе мера объёмом не работает ни при каком объёме (мешает STRESS_LOSS_LIMIT); задержка, объявленная позже августа 2037 г., не оставляет 4 месяцев на реакцию Earth-Flex | ПОСТ ZBO / ОП |
| Р6 | Emergency не успевает или не покрывает дефицит | любой год | детерминированные параметры канала: 6 недель, 80 т/год | контрактный резерв не заменяет физический (EXP-01): правило эквивалентности требует запаса на 6 недель ожидания и зарезервированной мощности ≥ 45-дневной нормы | физический резерв 45 дней как базовый режим; Emergency — только резерв, не базовый канал (≤ 2 лет подряд) | — (правило соблюдено во всех опубликованных планах) | ОП |
...
| Р8 | Превышение лимита CAPEX | до 2037 / до 2040 | детерминированно | P2z: 540 из 1 800 млн, запас 1 260 млн (EXP-01); у P4 запас лишь 10 млн | этапность опцион → реализация; ворота решения | пересмотр ISRU после 2040 требует нового лимита и данных | ФИН |
...
| Р10 | Совместные малые отклонения спроса, поставок и цен | 2038–2040 | сетка 0–10 % и обратный стресс по 9 направлениям (TEAM_ASSUMPTION) | фиксированный P2z: +1 % спроса нарушает резерв 2038–2040 (разрыв до 6,180 т), −1 % поставки Earth-New — разрыв 1,993 т; граница совместного шока исходного P2z ≈ 0,0000413 %, адаптированного P2z в стрессе ≈ 0,0000496 % (EXP-12); для P3 аналогично ≈ 0,000055 % (EXP-07) | запас 3,2 т нетто расширяет границу P2z до ≈ 0,39 % за +31,136 млн PV, защитный календарь — до ≈ 0,0089 % за +0,285 млн PV (EXP-12); для P3: запас +8,6 т = 83,662 млн PV, ранний ZBO + 0,1 т = 49,453 млн PV, частота отказов 100 % → 49 % (EXP-08, EXP-10) | остаточный отказ — резерв января 2040; вероятности не оцениваются | ОП / ФИН |
...
| Мера 3: защитный календарь (заранее) | при реализации опциона в 2035-01 поставки Earth-New назначаются на апрель–декабрь 2037 при прежнем годовом объёме 13,188 т и резерве 17,585 вместо 13,189 т/год: все проверки выполнены без задержки и при +3 мес. (+0,285 / +6,712 млн PV к плану без мер при том же шоке); при +6/+12 мес. разрыв резерва 4,125 / 12,812 т; физический запас 3,2 т нетто за +31,136 млн PV восстанавливает резерв при +3 мес., но не при +6/+12 (разрыв 3,097 / 9,611 т) |
| Мера 4: договор | этапные платежи, независимая приёмка, take-or-pay только на принятую мощность, неустойка за вменяемую задержку; не оценена моделью; справочно: при исключении недоступного объёма из оплаты экономия составила бы 23,409 / 46,818 / 93,636 млн (объём × 7,1) |
...
| Меры | заблаговременная адаптация: запас 35,446 т к 2038-01 вместо 30,822, Earth-New +4,759 т в 2037 и +39,266 т в 2038, Earth-Flex +49,893 т в 2039 и +35,263 т в 2040; дефицит 0, нарушений 0, PV 10 097,842 (+954,574 млн к неизменному плану в стрессе); перепланирование под высокий вариант: PV 10 238,585, дефицит 2,557 т, сервис ≥ 99,48 %, нарушений нет |
...
- **Р5 ZBO.** Лаг ввода 0–12 месяцев — допущение. Годовое отношение потерь к поступлению в стрессе превышает 2 % при лаге 10 месяцев (лаги 7–9 проходят), а до модернизации ёмкость 70 т ограничивает буферы. Цена задержки именно для выбранного P2z посчитана (EXP-17, `results/zbo_delay/`): ZBO меняет сразу два параметра — ёмкость 70 → 120 т и потери 4,5 % → 1,2 %, поэтому задержка бьёт не по обслуживанию (дефицита нет), а по резерву и потерям: разрывы 45-дневного резерва 1,676 / 3,352 / 7,672 т при 3 / 6 / 12 месяцах в BASE. **Ловушка учёта:** PV при задержке снижается, потому что CAPEX 180 млн и OPEX 12 млн/год платятся позже, — план выглядит дешевле и при этом перестаёт быть исполнимым, поэтому последствие измеряется стоимостью минимальной меры. Реакция через Earth-Flex (заказ к январю 2037, срок 4 месяца): 1,733 / 3,511 / 8,033 т за +10,832 / +22,268 / +54,100 млн PV, платится только при наступлении задержки. Более раннее решение о ZBO устраняет саму задержку: 0 / 0 / 12,346 млн PV, но премия 5,885 / 12,029 / 35,928 млн PV платится и без неё. Остаточный риск: при задержке 12 месяцев в обязательном стрессе мера объёмом не работает ни при каком объёме — потолок потерь 2 % действует с 2038 года, базовое хранилище с потерями 4,5 % при такой задержке работает ещё полгода 2038-го, и дополнительный объём только увеличивает потери; работает только устранение самой задержки. Дополнительно: приёмочные испытания и неустойка в договоре (предложено, моделью не оценено).
...
- **Р10.** У адаптированного P2z запас над нормой резерва в январе 2040 — 0,000469 т, поэтому любое совместное отклонение нарушает резерв: обратный стресс P2z даёт границу ≈ 0,0000413 % в BASE и ≈ 0,0000496 % в стрессе (EXP-12), как и у P3 (≈ 0,000055 %). Физический запас 3,2 т расширяет границу до ≈ 0,39 % за +31,136 млн PV, защитный календарь — до ≈ 0,0089 % за +0,285 млн PV; для P3 ориентир 49–84 млн PV. Границы детерминированы и не являются вероятностями.
...
4. Р9–Р10 фиксируют границы вывода: без физического запаса P2z не имеет запаса прочности к совместным отклонениям, а исключение ISRU — сознательный компромисс на горизонте 2035–2040.
