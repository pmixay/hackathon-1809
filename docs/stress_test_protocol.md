...
| EXP-12 | защита, чувствительность и обратный стресс выбранного P2z | физический запас Earth-Flex (минимум на сетке 0,1 т) и заранее согласованный «защитный календарь» Earth-New (апрель–декабрь 2037, резерв 17,585 т/год) против задержки 0/3/6/12 мес.; чувствительность P2z к спросу 0–10 %, недопоставке Earth-New 0–10 %, цене Core/Flex ×0,8…1,5; обратный стресс по 9 направлениям при нулевой задержке | как в EXP-09; фон BASE и адаптированный P2z в стрессе | P2z и два защищённых варианта | PV и разности при том же шоке и к исходному BASE, разрыв резерва, полный и физический проход, граница совместного шока | жёсткие нарушения и ориентиры сервиса; для физического прохода — резерв, ёмкость, сервис, потери |
| EXP-13 | меры реакции и буфера выбранного плана против задержки Earth-New | реакция через Earth-Flex после наблюдения (срок 4 мес.) и заблаговременный буфер; объём — минимальный (бисекция, шаг 0,001 т) | как в EXP-09 | P2z с задержкой 3/6/12 мес. | объём меры, PV, разности, резерв, сервис, премия без задержки | последствия события: жёсткие нарушения и ориентиры сервиса, кроме недоступности самого Earth-New |
...
|---|---:|---:|---:|:---:|---|
| P1 Earth-Core + Earth-Flex | 0 | 7 857,750 | 7,64 | нет: дефицит 97,548 т, сервис 0,750 в 2040 | дефицит 241,548 т, потери > 2 % |
| P2 + Earth-New | 360 | 8 779,822 | 7,97 | да | резерв 2038–2040, потери > 2 %, дефицит 95,917 т |
| P2z + Earth-New + ZBO | 540 | 8 729,774 | 7,90 | да | резерв 2038–2040, дефицит 95,917 т |
| P3 + ISRU + ZBO | 1 430 | 8 639,227 | 7,67 | да | резерв 2038–2040, дефицит 170,017 т |
| P4 полный портфель | 1 790 | 8 857,979 | 7,78 | да | резерв 2038–2040, дефицит 170,017 т |
...
|---|---|---|---|
| P2z | 9 143,268 / 95,917 т / резерв 2038–2040, сервис 2039–2040 | **10 097,842 / 0 / 0** | 17 переполнений хранилища |
| P3 | 9 031,995 / 170,017 т / резерв и сервис 2038–2040 | 10 637,025 / 0 / 0 | 25 переполнений |
| P4 | 9 213,751 / 170,017 т / резерв и сервис 2038–2040 | 10 578,598 / 0 / 0 | 25 переполнений |
...
| Высокий (+10…+25 %) | дефицит 234,917 т, минимальный общий сервис 0,800, резерв нарушен 2035–2040, PV 8 631,824 | нарушений 0, дефицит 2,557 т, минимальный сервис 0,9948, PV 10 238,585 |
...
**Порог смены победителя.** Сетка выше отвечает на вопрос «когда план перестаёт работать»; отдельно
рассчитан порог «когда другая архитектура становится дешевле». Оба плана перестраиваются под каждое
значение параметра, а точка равенства ищется делением отрезка пополам до объявленной точности.

| Метрика решения | Параметр | Диапазон поиска | Порог | Что он означает |
|---|---|---|---|---|
| PV в базовом сценарии | цена Earth-Core/Flex | ×0,5…×3,0 | ×2,033 (±0,001) | ниже этого множителя дешевле альтернатива с ISRU; выше — выбранный план, который опирается на Earth-New по неиндексируемой цене |
| PV в базовом сценарии | реальная ставка | 0…20 % | 12,60 % (±0,05 п.п.) | ниже ставки дешевле альтернатива с ISRU: её выигрыш по текущим затратам ещё не съеден ранним капиталом |
| PV адаптированного обязательного стресса (метрика выбора) | цена Earth-Core/Flex | ×0,5…×3,0 | не найден | выбранный план дешевле на всём диапазоне |
| PV адаптированного обязательного стресса (метрика выбора) | реальная ставка | 0…20 % | не найден | выбранный план дешевле на всём диапазоне |
| PV адаптированного обязательного стресса (метрика выбора) | капитальные затраты пилота ISRU | 100…1 250 млн | 667,6 млн (±0,5) | альтернатива с ISRU выигрывает, только если её капитальные затраты окажутся почти вдвое ниже заявленных 1 250 млн |

Вывод для защиты: по метрике, на которой основан выбор, решение не переворачивается ни ценой земных
каналов, ни ставкой дисконтирования — только существенно более дешёвым пилотом ISRU. По затратам
базового сценария альтернатива с ISRU действительно дешевле, и это не скрывается: она проигрывает
на стрессе, а не на спокойном сценарии.
...
исправить уже нельзя; PV 11 135,633 против 10 637,025 у заблаговременной адаптации. Разница 498,608 млн не равна
цене предвидения: конечный запас 55,295 т против 21,755 т, включая 33,947 т дополнительного Emergency в 2040.

**Автоматическая проверка правила реакции.** План реакции сравнивается с планом до события решение за
решением: инвестиции и начальный запас должны совпадать, а каждое изменённое решение — быть выполнимым
после наблюдения, то есть месяц размещения заказа (месяц поставки минус срок поставки) не раньше месяца
наблюдения; договорной объём года, закончившегося до наблюдения, не пересматривается. Проверка выполняется
при каждом прогоне эксперимента и останавливает его при нарушении; отчёт со всеми 53 изменёнными решениями
и датой «заказать до» для каждого прилагается к результатам. Заблаговременно адаптированный план ту же
проверку не проходит — и не должен: он по построению использует знание сценария заранее. Именно поэтому
его стоимость сравнивается с реакцией отдельно, а не выдаётся за достижимую реакцию.
...
### EXP-12. Защита, чувствительность и обратный стресс P2z (BASE, копия данных)

| Мера | Без задержки: ΔPV | +3 мес.: ΔPV к «без мер» при том же шоке / полный проход | +6 / +12 мес. |
|---|---:|---|---|
| Физический запас Earth-Flex 3,2 т нетто (июль–декабрь 2037) | +31,136 | +31,136 / резерв восстановлен, но недоступность Earth-New остаётся фактом события | разрыв резерва 3,097 / 9,611 т |
| Защитный календарь Earth-New: поставки апрель–декабрь 2037, резерв 17,585 т/год, согласован в 2035-01 | +0,285 | +6,712 / все проверки выполнены | не защищает: разрыв 4,125 / 12,812 т |

Чувствительность фиксированного P2z (BASE): +1 % спроса нарушает резерв 2038–2040 (разрыв до 6,180 т), +10 % даёт дефицит 47,917 т; −1 % поставки Earth-New — разрыв 1,993 т в 2039–2040; цена Core/Flex ×0,8 снижает PV на 376,713 млн. Обратный стресс при нулевой задержке (совместный рост спроса и недопоставка Earth-New в 2038–2040, 9 направлений): граница исходного P2z ≈ 0,0000413 %, с запасом 3,2 т ≈ 0,3916 %, с защитным календарём ≈ 0,0089 %, адаптированного P2z в обязательном стрессе ≈ 0,0000496 %; первое нарушение — резерв января 2040. Календарь защищает от определённой задержки ввода, но не от существенной недопоставки после ввода; вероятности не оцениваются.

### EXP-09 и EXP-13. Задержка Earth-New: реакция после наблюдения и буфер заранее (BASE, копия данных)
...
|---|---:|---:|---:|---|
| P2z | 8 729,774 | 9 200,296 | +470,892 | без изменений |
| P3 | 8 639,227 | 9 091,370 | +452,513 | без изменений |
| P4 | 8 857,979 | 9 273,495 | +415,516 | без изменений |
...
только по CSV (`python tests/independent_recalc.py <каталог>`) не использует движок. Для EXP-07–13 команда
