...
const base=await load('results/alternatives/P3_isru_zbo_BASE/result.json');
const stress=await load('results/alternatives/P3_isru_zbo_MANDATORY_STRESS/result.json');
const adapted=await load('results/stress/P3_isru_zbo_adapted_MANDATORY_STRESS/result.json');
...
const slides=[
 ...
 {title:'Сравнение исполнимых планов в BASE',chart:{categories:['P2 New','P2z New + ZBO','P3 ISRU + ZBO','P4 полный'],series:[{name:'PV, млн у.е.',values:alternatives.map(r=>Math.round(r.kpi.pv_cost_mln)),fill:'#176a5b'}]},caption:'P1 не проходит ограничения и исключён из ценового ранжирования.',source:'results/alternatives/*_BASE/result.json'},
 {title:'P3 в обязательном стрессе',lines:[`Неизменный план: дефицит ${stress.kpi.shortage_total_t.toFixed(1)} т.`,`Нарушения резерва: ${stress.violations.filter(v=>v.rule_id==='RESERVE_45D').map(v=>v.year).join(', ')}.`,'Спрос +15 % с 2038. Цены A/B +25 % в 2038–2039.','Поставка ISRU: 55 % в 2038 и 75 % в 2039, без повторного множителя надёжности.'],source:'results/alternatives/P3_isru_zbo_MANDATORY_STRESS/result.json; configs/scenarios/mandatory_stress.yaml'},
 {title:'Заблаговременная адаптация P3',chart:{categories:['BASE','Стресс, план прежний','Стресс, адаптация'],series:[{name:'PV, млн у.е.',values:[base,stress,adapted].map(r=>Math.round(r.kpi.pv_cost_mln)),fill:'#176a5b'}]},caption:`Адаптация: дефицит ${round(adapted.kpi.shortage_total_t)} т, жёстких нарушений ${adapted.kpi.hard_violations}. Это разные решения, не гарантия робастности.`,source:'results/alternatives/P3_isru_zbo_BASE/result.json; results/alternatives/P3_isru_zbo_MANDATORY_STRESS/result.json; results/stress/P3_isru_zbo_adapted_MANDATORY_STRESS/result.json'},
 {title:'Риски и правила реакции',lines:['Буфер запаса нужен до начала года: поздняя реакция не исправит резерв на 1 января.','Дозаказ Flex требует 4 месяцев, Emergency — 6 недель.','EXP-07–10: обратный стресс, меры защиты, задержка Earth-New и Монте-Карло.','Монте-Карло: N = 10 000, seed = 203510. Распределения TEAM_ASSUMPTION, вероятности не калиброваны.'],source:'results/reaction/summary.md; results/monte_carlo/summary.md; docs/risk_register.md; docs/stakeholders.md'},
 ...
 {title:'Бюджет P3 по годам',chart:{categories:base.finance.map(r=>String(r.year)),series:[{name:'Всего, млн',values:base.finance.map(r=>Math.round(r.total_mln)),fill:'#176a5b'},{name:'CAPEX, млн (в составе итого)',values:base.finance.map(r=>r.capex_mln),fill:'#d5a747'}]},caption:'ISRU: оплата 2036-01. ZBO: оплата 2037-07. CAPEX P3 до 2037: 1 430 млн.',source:'results/alternatives/P3_isru_zbo_BASE/financial_breakdown.csv; docs/roadmap_budget.md'},
...
for(let i=0;i<slides.length;i++){
 ...
 text(s,`${i+1} / ${slides.length}`,1140,670,85,28,17,i===0?'#d0e3df':'#6a807a');
