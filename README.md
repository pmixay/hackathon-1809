...
python -m pytest -q                             # 405 тестов: контрольные примеры V01–V10, движок, ошибочный ввод, границы, API, UI, verify всех результатов
...
> # https://terra.arbuz.lol/
>
> ### Страница решения развёрнута и открыта: проверено 19.09.2026, HTTP 200
>
> Это **статическая страница решения**. Расчётный контур (пульт оператора, `/console`, `/api/*`)
> там не развёрнут и отвечает 404 — он запускается локально: `python -m terraplan ui`.
> Кейс допускает такую поставку: требуется «доступ к системе **или** файлы для её запуска,
