...
| `sensitivity/` | EXP-04 | `sweep_*.csv`, `thresholds.csv`, `tornado.csv`, `ranking_by_discount_rate.csv`, `crossover_P2z_vs_P3.csv` (порог смены победителя), `summary.md` |
| `reaction/` | EXP-06 | реакция после наблюдения (Earth-Flex / Emergency после 2038-03) против плана без изменений и заблаговременной адаптации; `compare_*.md`, `response_rule_check.md` (проверка, что реакция не опережает наблюдение) |
...
| `risk_assessment.csv` / `.md` | реестр рисков | последствия всех рисков, пересчитанные расчётным контуром и сверенные с реестром |
