...
(async()=>{
 ...
 await page.locator('#compare-run').click();await page.waitForSelector('#comparison table');
 assert.match(await page.locator('#comparison').innerText(),/shortage_total_t/);
 ...
 await page.locator('#preset').selectOption('P3_isru_zbo_adapted');await page.locator('#load').click();await run();
 ...
 assert.match(await page.locator('#result-title').innerText(),/P3_isru_zbo_adapted/);
